...
# =========================
if len(sys.argv) != 2:
    print("Usage: python beta_variabile.py <seed>")
    sys.exit(1)
...
def apply_beta_properties(DFN, beta):
    result = DFN.generate_hydraulic_values(
        "aperture",
        "correlated",
        {"alpha": 5e-4, "beta": beta}
    )
...
try:
    ...
    # BETA LOOP
    # =========================
    for beta_id, beta in enumerate(beta_values, start=1):

        print(f"\n  -> beta {beta}")

        apply_beta_properties(BACKBONE, beta)
        ...
        for direction in directions:
            ...
            fname = f"{BACKBONE.local_jobname}_seed{seed}_beta{beta_id}_{direction}.txt"
